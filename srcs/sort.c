...
/*   Updated: 2022/05/06 12:06:46 by sam              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	*/	return (-1);
	...
	if (r_o != *stack_buf)
	{
		...
		*/
		...
		//	ft_printf("first_keep=%d last_keep=%d *stack_buf =%d \n", first_keep->nb, last_keep->nb, (*stack_buf)->nb);
			...
			while (first_keep != *stack_buf)
			{
				...
				{
				//	ft_printf("which push de la ligne 185 return -1\n");
				//	ft_printf("first_keep=%d last_keep=%d *stack_buf =%d \n", first_keep->nb, last_keep->nb, (*stack_buf)->nb);
					return (-1);
				}
				//ft_printf(" push pour merger\n");
				if (count_elem_in_stack(*stack_buf) > 1)
				{
					if (which_rr(stack_buf, !src_is_a) == -1)
					{
					//	ft_printf("which rr de la ligne 192 return -1\n");
					//	ft_printf("first_keep=%d last_keep=%d *stack_buf =%d \n", first_keep->nb, last_keep->nb, (*stack_buf)->nb);
						return (-1);
					}
				}
				//ft_printf("RR pour trier le merge\n");

			}
			if (which_push(stack_buf, stack_src, !src_is_a) == -1)
				return (-1);
			//ft_printf("here last push, stack fully sorted\n");
